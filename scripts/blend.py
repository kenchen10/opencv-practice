...
for i in range(5, 0, -1):
    ...
    laplassian_apple = cv2.subtract(gp_apple[i-1], gaussian_extended_apple)
    lp_apple.append(laplassian_apple)
    gaussian_extended_orange = cv2.pyrUp(gp_orange[i])
    laplassian_orange = cv2.subtract(gp_orange[i-1], gaussian_extended_orange)
    lp_orange.append(laplassian_orange)
